...
if($ussdRequest != NULL) 
{
	...
	if($ussdRequest->Sequence == "4")
	{
		...
		#if user did entered the valid momo number..........
		elseif( is_numeric($ussdRequest->Message)) //trim(strlen($ussdRequest->Message) == 12) &&
		{
			$contestant_ids  = "";
	        $contestant_nam  = "";
	        $voteidentifier  = "";
			...
			$mtn_update = "UPDATE `track_process` SET `amount`='".$ussdRequest->ClientState."', `payer_phone` = '".$ussdRequest->Message."', contestant_id = '".$contestant_ids."', nominee_name = '".$contestant_nam."', `voteidentifier` = '$voteidentifier', `user_entry` = '$user_entry' WHERE (`initiator`='".$ussdRequest->Mobile."') ";
    		$test_run = $conn->query($mtn_update);
			
			#alert user to authorize payment then pass session data to payment API.............
			$ussdResponse->Message = "Please authorize payment for '".$contestant_nam."' on ".$ussdRequest->Message.". Thank you. Keep voting.";
		 	$ussdResponse->Type    = "Release";
		...
		{	
			...
			#update session data and allow the user to retry again
			...
			$ussdResponse->Message = "Sorry, entry invalid.\nEnter valid mobile money number.";
