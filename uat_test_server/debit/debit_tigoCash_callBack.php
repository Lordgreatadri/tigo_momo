...
require_once 'MCC_data_auth.php';
require_once 'dbconfig.php';

$data_Obj = new data_functions();

$tigo_Obj = new MCC_data_auth();
...
if($correlationID && trim(!empty($correlationID))) 
{
    ...
    $update = $data_Obj->update_PaymentRequest_status($transactionID, $code, $description, $correlationID);
// var_dump($update);
   $log = $data_Obj->logPaymentResponse($transactionID, $external_transaction_id, $correlationID, $status, $code, $momo_number, $description, $amount);
   // var_dump($log);

   $tigo_Obj->airteltigo_cash_callback_response($transaction_id, $correlationID, $status, $code, $description);
    // return $data;
}
